		...
		if (actor != null && actor.solver != null)
		{
			...
			if (actor.InSolver){
				...
				for (int i = 0; i < handledParticleIndices.Count; ++i)
				{
					...
					Oni.SetParticleInverseMasses(actor.solver.Solver,invMass,1,solverParticleIndex);
				}
			}
		}
	}
	...
	void Actor_solver_OnFixedParticlesUpdated (object sender, System.EventArgs e)
	{
		if (actor.InSolver){
			...
			for (int i = 0; i < handledParticleIndices.Count; ++i){
				...
				Oni.SetParticleVelocities(actor.solver.Solver,vel,1,solverParticleIndex);
				Oni.SetParticleInverseMasses(actor.solver.Solver,invMass,1,solverParticleIndex);

				// set particle position:
				pos[0] = transform.TransformPoint(handledParticlePositions[i]);
				Oni.SetParticlePositions(actor.solver.Solver,pos,1,solverParticleIndex);
