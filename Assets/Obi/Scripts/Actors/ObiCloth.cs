...
public class ObiCloth : ObiActor
{
	...
	private void ApplyTearing(){
		...
		if (tearedEdges.Count > 0){
			...
			Oni.GetParticlePositions(solver.Solver,solverPositions,solver.maxParticles,0);
			...
			Oni.SetParticlePositions(solver.Solver,solverPositions,solver.maxParticles,0);
		...
		if (skinnedMeshRenderer != null){
			...
			for(int i = 0; i < particleIndices.Count; i++){
				...
				if (invMasses[i] == 0){
					...
					Oni.SetParticlePositions(solver.Solver,skinPos,1,particleIndices[i]);
				}
	...
	public void Tear(int constraintIndex,Vector4[] solverPositions,MeshBuffer buffer){
		...
		Oni.GetParticleVelocities(solver.Solver,velocity,1,particleIndices[splitIndex]);
		Oni.SetParticleVelocities(solver.Solver,velocity,1,newParticle[0]);
		...
		Oni.SetParticleInverseMasses(solver.Solver,new float[]{invMasses[splitIndex]},1,newParticle[0]);
		Oni.SetParticleSolidRadii(solver.Solver,new float[]{solidRadii[splitIndex]},1,newParticle[0]);
		Oni.SetParticlePhases(solver.Solver,new int[]{phases[splitIndex]},1,newParticle[0]);
