...
namespace Obi{
	...
	public class ObiAerodynamicConstraints : ObiConstraints
	{
		...
		protected override void OnAddToSolver(object info){
			...
			// Add constraints:
			Oni.SetAerodynamicConstraints(solver.Solver,solverIndices,aerodynamicNormals.ToArray(),wind.ToArray(),aerodynamicCoeffs.ToArray(),ConstraintCount,constraintOffset);
		...
		public override void PushDataToSolver(ObiSolverData data){ 
			...
			if (data.aerodynamicConstraintsData == ObiSolverData.AerodynamicConstraintsData.ACTIVE_STATUS)
				...
				Oni.UpdateAerodynamicNormals(actor.solver.Solver,aerodynamicNormals.ToArray(),ConstraintCount,constraintOffset);
			else
				Oni.SetAerodynamicConstraints(actor.solver.Solver,solverIndices,aerodynamicNormals.ToArray(),wind.ToArray(),aerodynamicCoeffs.ToArray(),ConstraintCount,constraintOffset);
