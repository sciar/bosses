...
namespace Obi{
	...
	public class ObiVolumeConstraints : ObiConstraints 
	{
		...
		protected override void OnAddToSolver(object info){
			...
			{
				solverFirstTriangle[i] = Oni.GetVolumeTriangleCount(actor.solver.Solver) + firstTriangle[i];
			}

			Oni.SetVolumeConstraints(solver.Solver,solverIndices,
												  solverFirstTriangle,
		...
		public override void PushDataToSolver(ObiSolverData data){ 
			...
			Oni.SetVolumeConstraints(actor.solver.Solver,solverIndices,
												  solverFirstTriangle,
											      numTriangles.ToArray(),
												  restVolumes.ToArray(),
												  pressureStiffness.ToArray(),
