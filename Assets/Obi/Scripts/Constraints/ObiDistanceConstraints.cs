...
public class ObiDistanceConstraints : ObiConstraints
{
	...
	protected override void OnAddToSolver(object info){
		...
		// Add constraints:
		Oni.SetDistanceConstraints(solver.Solver,solverIndices,restLengths.ToArray(),stiffnesses.ToArray(),ConstraintCount,constraintOffset);
	}
	...
	public override void PushDataToSolver(ObiSolverData data){ 
		...
		Oni.SetDistanceConstraints(actor.solver.Solver,solverIndices,scaledRestLengths,stiffnesses.ToArray(),ConstraintCount,constraintOffset);
	...
	public override void PullDataFromSolver(ObiSolverData data){
		if (actor != null && actor.solver != null && stretching != null){
			...
			if ((data.distanceConstraintsData & ObiSolverData.DistanceConstraintsData.DISTANCE_STRETCH) != 0){
				Oni.GetDistanceConstraintsStretching(actor.solver.Solver,stretching,ConstraintCount,ConstraintOffset);
			}
		}
	}	
